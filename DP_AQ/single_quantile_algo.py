...
def racing_sample(log_terms, rng):
    ...
    """
    return np.argmin(np.log(np.log(1.0 / rng.uniform(size=log_terms.shape))) - log_terms)


def single_quantile(sorted_array, bounds, quantile, epsilon, swap, rng):
    a, d = bounds
    n = len(sorted_array)
    ...
    idx_left = racing_sample(np.log(intervals) + (epsilon / (2.0 * sensitivity) * utility), rng)
    return rng.uniform(sorted_array[idx_left], sorted_array[idx_left + 1])
